...
def main(opts):
    ...
    txt_db_list = opts.txt_db.split(";")
    print(txt_db_list)

    if exists(opts.checkpoint):
        ckpt_file = opts.checkpoint  # /mnt2/user15/VILLA/vcr_data/pretrained/villa-base-vcr-2nd-stage.pt
    else:
    ...
    # prepare model train-multi
    model = UniterForVCRPretraining.from_pretrained(
        f'{opts.output_dir}/log/model.json', checkpoint, img_dim=IMG_DIM, use_adapter=opts.use_adapter,
    ...
    #     img_dim=IMG_DIM, use_adapter=False)
    #
    ...
    val_list = []
    val_pred_list = []
    for txt_db in txt_db_list:
        eval_txt_db = VcrTxtTokLmdb(txt_db, -1)
        ...
        _, results, check_correct, tot_pred_list = evaluate(model, eval_dataloader)
        val_list.append(torch.tensor(check_correct).long())
        val_pred_list.append(torch.tensor(tot_pred_list).long())
        ...
        if hvd.rank() == 0:
            ...
            #probs_df.to_csv(f'{result_dir}/results_{opts.checkpoint}_{sub_db}_all.csv')
    assert len(val_list[0]) == len(val_list[1])
    assert len(val_pred_list[0]) == len(val_pred_list[1])
    val_origin = val_list[0] & val_list[1]
    val_preds = val_pred_list[0] != val_pred_list[1]
    val_preds2 = val_origin & val_preds.long()

    tot_acc = torch.sum(val_origin).item() / len(val_origin) *100
    tot_reliance = torch.sum(val_preds2).item() / torch.sum(val_preds).item() * 100
    LOGGER.info(f"Total consistency in VCR-CE : {round(tot_acc, 2)} %")
    LOGGER.info(f"Total reliance in VCR-CE : {round(tot_reliance, 2)} %")
...
def evaluate(model, eval_loader):
    ...
    st = time()
    results = {}
    tot_qa_check_correct = []
    tot_qa_check_reliance = []
    for i, batch in enumerate(eval_loader):
        qids = batch['qids']
        qa_targets, qar_targets = batch['qa_targets'], batch['qar_targets']
        scores = model(batch, compute_loss=False, task="vcr_tdt")

        #scores = torch.nn.functional.normalize(scores)
        #scores_itm = model(batch, compute_loss=False, task="itm_tdt")
        #scores_itm = torch.nn.functional.normalize(scores_itm)

        #scores = scores * 0.2 + scores_itm * 0.8
        scores = scores[:, 1:]
        ...
        if torch.max(qa_targets) > -1:
            ...
            curr_qa_score, curr_qar_score, curr_score, outputs_qa, matched_qa = compute_accuracies(
                scores[:, :4], qa_targets, qar_scores, qar_targets)
            tot_qar_score += curr_qar_score
            tot_qa_score += curr_qa_score
            tot_score += curr_score
            print(matched_qa.tolist())
            try:
                tot_qa_check_correct.extend(matched_qa.tolist())
                tot_qa_check_reliance.extend(outputs_qa.tolist())
            except:
                tot_qa_check_correct.extend([matched_qa])
                tot_qa_check_reliance.extend([outputs_qa])
        for qid, score in zip(qids, scores):
            results[qid] = score.cpu().tolist()
        n_ex += len(qids)
        val_pbar.update(1)
    assert len(tot_qa_check_correct) == n_ex
    ...
    return val_log, results, tot_qa_check_correct, tot_qa_check_reliance
...
def compute_accuracies(out_qa, labels_qa, out_qar, labels_qar):
    ...
    return n_correct_qa, n_correct_qar, n_correct_joined, outputs_qa, matched_qa
