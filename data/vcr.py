...
class VcrDetectFeatTxtTokDataset(DetectFeatTxtTokDataset):
    def __init__(self, txt_db, img_db_gt=None, img_db=None):
        ...
        # if list(txt2img.keys())[0].startswith("train"):
        #     name = "train"
        # else:
        #     name = "val"
        ...
        if self.img_db and self.img_db_gt:
            self.lens = [tl+self.img_db_gt.name2nbb[txt2img[id_][0]] +
                         self.img_db.name2nbb[txt2img[id_][1]] for tl, id_ in zip(txt_lens, self.ids)]
    ...
    def _get_img_feat(self, fname_gt, fname):
        if self.img_db and self.img_db_gt:
            # ########
            # fname_gt = fname_gt.replace("vcr_gt_train", "nlvr2")
            # fname = fname.replace("vcr_train", "nlvr2")
            # ########
            #
            # fname_gt = fname_gt.replace("vcr_gt_val", "nlvr2")
            # fname = fname.replace("vcr_val", "nlvr2")
