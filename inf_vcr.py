...
def main(opts):
    ...
    sub_db = "original" if "original" in opts.txt_db else "counter"
    ...
    if hvd.rank() == 0:
        with open(f'{result_dir}/'
                  f'results_{opts.checkpoint}_{sub_db}_all_villa.json', 'w') as f:
...
def evaluate(model, eval_loader):
    ...
    for i, batch in enumerate(eval_loader):
        ...
        scores = model(batch, compute_loss=False, task="vcr")
        scores = scores[:, 1:]
