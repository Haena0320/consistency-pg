...
from .adapter_layer import BertLayer, BertPooler
...
class UniterEncoder(nn.Module):
    def __init__(self, config, use_adapter):
        super().__init__()
        layer = BertLayer(config, use_adapter)
        self.layer = nn.ModuleList([copy.deepcopy(layer)
                                    for _ in range(config.num_hidden_layers)])

    def forward(self, input_, attention_mask,task,
                output_all_encoded_layers=True):
        all_encoder_layers = []
        hidden_states = input_
        for layer_module in self.layer:
            hidden_states = layer_module(hidden_states, attention_mask, task)
...
class UniterModel(UniterPreTrainedModel):
    ...
    def __init__(self, config, img_dim, use_adapter):
        ...
        self.encoder = UniterEncoder(config, use_adapter)
        ...
        if self.use_person_ids:
            print("!!! we'll add person_ids embeddings to visual embedddings !!!!")
    def get_input_embeddings(self):
        return self.embeddings.word_embeddings

    def set_input_embedings(self, value):
    ...
    def _compute_img_txt_embeddings(self, input_ids, position_ids,
                                    ...
                                    person_ids=None, boxes_mask=None):
        ...
        if  self.use_person_ids:
            ## visualcomet PG as a baseline
            pack_person_ids = pack_sequence(person_ids, boxes_mask)
            person_emb = self.embeddings.word_embeddings(pack_person_ids)
            person_emb = pad_sequence(person_emb, boxes_mask.sum(1).tolist())
            assert person_emb.size() == img_emb.size()
            img_emb += person_emb
    ...
    def forward(self, input_ids, position_ids,
                ...
                output_all_encoded_layers=True,
                txt_type_ids=None, img_type_ids=None, person_ids=None, boxes_mask=None, task=None):
        ...
        if input_ids is None:
            ...
            embedding_output = self._compute_img_txt_embeddings(
                ...
                img_masks, txt_type_ids, img_type_ids, person_ids=person_ids, boxes_mask=boxes_mask)


        encoded_layers = self.encoder(
            embedding_output, extended_attention_mask, task,
            output_all_encoded_layers=output_all_encoded_layers)

        if not output_all_encoded_layers:
            encoded_layers = encoded_layers[-1]

        if not task[0] in ["vcr_tdt", "itm_tdt"]:
        ...
        negative_embedding_output = embedding_output * (1-knowledge_mask).expand_as(embedding_output)
        ...
        mask_encoded_layers = self.encoder(
            masked_embedding_output, extended_attention_mask, task,
            output_all_encoded_layers=output_all_encoded_layers)

        negative_encoded_layers = self.encoder(
            negative_embedding_output, extended_attention_mask, task,
            output_all_encoded_layers=output_all_encoded_layers)

        if not output_all_encoded_layers:
            mask_encoded_layers = mask_encoded_layers[-1]
            negative_encoded_layers = negative_encoded_layers[-1]

        return encoded_layers, mask_encoded_layers, negative_encoded_layers, knowledge_mask
